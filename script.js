...
(function(){
  ...
  var N=1900,sp=new Float32Array(N*3),ss=new Float32Array(N),sc=new Float32Array(N*3);
  for(var i=0;i<N;i++){
  ...
  var sMat=new THREE.PointsMaterial({size:3.2,sizeAttenuation:true,map:sTex,vertexColors:true,transparent:true,opacity:1.3,blending:THREE.AdditiveBlending,depthWrite:false,alphaTest:0.01});
  var starField=new THREE.Points(sG,sMat);S.add(starField);

  // CURSOR GLOW OVERLAY — stars brighten near the cursor (additive layer on top of starField)
